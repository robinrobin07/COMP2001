...
while [ $# -gt 0 ]; do
    ...
    -v | -verbose)
        ...
        ;;
    *)
        # CUSTOM INTERFACE TO BE CHECKED
        [ "$customInterfaceRequested" == "" ] && customInterfaceRequested="$1"
        ;;
    esac
...
# interface="eno1"
if [ "$customInterfaceRequested" == "" ]; then
    echo "NO CUSTOM INTERFACE INFORMATION REQUESTED"
else

    interface=$customInterfaceRequested
...
for interface in $(nmcli device status | awk '{print $1}' | awk '(NR>1)' | head -n -1); do
